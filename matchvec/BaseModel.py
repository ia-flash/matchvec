    ...
    def download_model_folder(self, dst_path, src_path):
        ...
        for file in self.files:
            if not osp.isfile(osp.join(dst_path, file)):
                ...
                print("In the bucket %s"%src_path)
                print("To %s locally"%dst_path)
                s3 = boto3.resource('s3')
                myobject = s3.Object(bucket_name, osp.join('model',src_path, file))
                myobject.download_file(osp.join(dst_path, file))
                print("Downloading ok\n")
            else:
                print("Model already locally at %s"%osp.join(dst_path, file))
